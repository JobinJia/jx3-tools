...
- `services/hotkey/` - `keymap.rs` (key label → scancode/VK/shortcut-string mapping, the single source of truth), `keys.rs` (Interception kernel injection — opens the `\\.\interception00`–`09` **keyboard** devices directly and sends `IOCTL_WRITE` + `KEYBOARD_INPUT_DATA`; `interception.dll` is deliberately NOT used, see below), `driver.rs` (driver install/uninstall/state detection + mouse-filter cleanup, REG_MULTI_SZ helpers unit-tested cross-platform), `window.rs` (window enumeration / PostMessage), `config.rs` (validation + JSON persistence), `types.rs`. **Driver install is in-app and user-initiated only** (`install_hotkey_driver`, hotkey-page banner) — NEVER auto-install from NSIS: `install-interception.exe /install` always installs keyboard **and mouse** class filters (no keyboard-only switch) and the mouse filter once bricked a user's mouse. The install flow runs the official installer, then **before any reboot** strips `mouse` from the Mouse class `UpperFilters` (a filter removed before reboot never loads); if cleanup fails it auto-rolls-back via `/uninstall`. `interception.dll` can't be used because its `create_context` requires all 20 devices (10 kbd + 10 mouse) to open — impossible under keyboard-only install; hence the direct-device client in `keys.rs`
